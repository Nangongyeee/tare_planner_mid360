...
void scanHandler(const sensor_msgs::PointCloud2::ConstPtr& scanIn)
{
  ...
  int scanDataSize = scanData->points.size();
    tf::Quaternion q;
  q.setRPY(M_PI, 0, 0); // 绕 Z 轴旋转 180 度
  
  for (int i = 0; i < scanDataSize; i++)
  {
    pcl::PointXYZI& point = scanData->points[i];
    
    tf::Vector3 pointVec(point.x, point.y, point.z);

    // 将四元数转换为旋转矩阵，并旋转向量
    tf::Matrix3x3 m(q);
    pointVec = m * pointVec;  // 旋转点
...
int main(int argc, char** argv)
{
  ...
  ros::Subscriber subScan = nh.subscribe<sensor_msgs::PointCloud2>("/livox/lidar", 2, scanHandler);
